...
if ~isfield(EXPERIMENT, 'DATA_SET_FOLDS')
EXPERIMENT.DATA_SET_FOLDS = 1;
end
% Load the data.
for fold=1:EXPERIMENT.DATA_SET_FOLDS
...
for m_id = 1:length(EXPERIMENT.M)
    ...
    for trial_id = 1:EXPERIMENT.NUM_TRIALS
        disp(sprintf('testFullRBF: m = %d, trial %d.', m, trial_id));
        %----------------------------------------
	...
	hyp = minimize(hyp, @gp, -100, @infExact, [], {@covSEiso}, likfunc, trainX, trainY);
        hypTime = toc(hypTic);
	...
	disp('Predicting for test inputs.');
        [mF s2F] = gp(hyp, @infExact, [], {@covSEiso}, likfunc, trainX, trainY, testX);
        ...
        save(sprintf('%sresultsFullRBF_%s_fold%d', EXPERIMENT.RESULTS_DIR, EXPERIMENT.DATASET, fold), 'resultsSoD');
    end
end
end
