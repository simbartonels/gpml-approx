...
if nargout>1                                % do we want the marginal likelihood
  ...
  if nargout>2                                         % do we want derivatives?
    ...
    while chol_bad  ~= 0
      if chol_iter > 20
        error('Non positive-definite matrix in FITC derivative calculation');
      end
      [C chol_bad] = chol(Kuu+eye(nu)*snu2*2^chol_iter+WW);
      chol_iter = chol_iter+1;
    end
    if lastChol > chol_iter
      chol_iter = lastChol;
    end
    W = C'\Ku;
